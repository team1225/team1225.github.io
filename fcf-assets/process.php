...
if(doRecaptchaCheck()) {
    ...
    if (isset($_POST['recaptcha-token'])) {
        ...
        exitFail("reCAPTCHA value missing, form may be misconfigured.");
    }
    ...
    if ($response_success === false) {
        $message = "reCAPTCHA rejected this request.<br>";
        exitError($message);
    }
...
try {
    ...
    if (filesExist()) {
        foreach ($_FILES as $upload_field) {
            for ($i = 0; $i < count($upload_field['name']); $i++) {
                if(isset($upload_field['tmp_name'][$i]) && strlen($upload_field['tmp_name'][$i]) > 1) {
                    $mail->AddAttachment($upload_field['tmp_name'][$i], $upload_field['name'][$i]);
                }
            }
        }
    }
